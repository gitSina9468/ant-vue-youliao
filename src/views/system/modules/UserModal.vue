      ...
      <a-form :form="form">
        ...
        <a-form-item label="岗位分配" :labelCol="labelCol" :wrapperCol="wrapperCol" >
          <a-select
            ...
            v-decorator="[ 'jobId', {}]"
            >
            <a-select-option v-for="(job,jobindex) in jobList" :key="jobindex.toString()" :value="job.id">
              {{ job.job }}
            </a-select-option>
          </a-select>
        </a-form-item>
        </template>
        <a-form-item label="头像(最大5M)" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-upload
...
<script>
  ...
  import {addUser,editUser,queryUserRole,queryRoleAll,queryJobAll,checkUsername } from '@/api/api'
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          ...
          nickName:{rules: [{ required: true, message: '请输入用户名称!' }]},
    ...
    methods: {
      ...
      edit (record) {
        ...
        that.$nextTick(() => {
          that.form.setFieldsValue(pick(this.model,'userName','sex','nickName','email','phone','jobId'))
        });
        // 调用查询用户对应的部门信息的方法
      ...
      handleChange (info) {
        ...
        if (info.file.status === 'done') {
          ...
          if(response.code == 200){
            ...
            this.$message.warning(response.remark);
          }
        }
      },
      getAvatarView(){
